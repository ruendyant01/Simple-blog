...
class BlogController extends Controller
{
    ...
    {
        extract($req->all());
        $blog = auth()->user()->blogs()->create(["title" => $title, "body" => $body, "image" => $image->getClientOriginalName(), "published_at" => isset($published_at) ?: null ]);
    ...
    public function update(Request $req, Blog $id) {
        if($req->has('image')) {
            ...
            $id->deleteImage($id->image->getClientOriginalName());
